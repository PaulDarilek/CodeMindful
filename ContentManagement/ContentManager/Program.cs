...
public static class Program
{
    private static CommandParser Parser { get; } 
        = new CommandParser([
            ...
            new CommandType(nameof(HashCodesForDrive), HashCodesForDrive) { MinParms = 1, MaxParms = 10, Description = "Compute Hashes Missing from Database for one or more Drive Letters"  },
            new CommandType(nameof(VerifyFilesExist), VerifyFilesExist) { MinParms = 1, MaxParms = 10 , Description = "Verify Files already Scanned for one or more Drive Letters" },
            //new CommandType(nameof(Restore), Restore) { MinParms = 0, MaxParms = 100 , Description = "" },
            new CommandType(nameof(DeleteFile), DeleteFile) { MinParms = 1, MaxParms = 100 , Description = "Delete files at a given path" },
        ]);
    ...
    private static void VerifyFilesExist(IEnumerable<string> driveLetters) {
        ...
        {
            var drive = Manager?.Drives.Values.FirstOrDefault(d => driveLetter.Equals(d.DriveLetter, StringComparison.OrdinalIgnoreCase));
            if (drive != null)
            {
