...
{
    public class ContentManager(IContentStore contentStore)
    {
        public IContentStore Database { get; } = contentStore;
        public FileManager DriveManager { get; } = new FileManager(contentStore);
        ...
        public int AddFiles(IEnumerable<FileInfo> files, bool computeHash = false)
        {
            ...
            foreach (var file in files)
            {
                ...
                if (found.Length == 0)
                {
                    var drive = DriveManager.GetDrive(file);
        ...
        public Models.File[] Find(FileInfo file)
        {
            ...
            ArgumentOutOfRangeException.ThrowIfNullOrWhiteSpace(file.DirectoryName);
