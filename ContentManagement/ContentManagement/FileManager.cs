...
namespace ContentManagement
{
    ...
    public class FileManager
    {
        ...
        private IContentStore Database { get; }
        private static JsonSerializerOptions JsonOptions { get; } = new JsonSerializerOptions { WriteIndented = true };

        public FileManager(IContentStore database)
        {
            Database = database;
        ...
        public void LoadCurrentDrives()
        {
            foreach (var info in DriveInfo.GetDrives())
            {
                if (info.IsReady)
                {
                    ...
                    if (drive != null)
                    {
                        Drives[drive.DriveLetter + Path.DirectorySeparatorChar] = drive;
                        try
                        {
                            var jsonFile = new FileInfo(Path.Combine(info.Name, "DriveInfo.json"));
                            if (info.IsReady == true)
                            {
                                string json = JsonSerializer.Serialize(drive, options: JsonOptions);
        ...
        {
            if(Drives.TryGetValue(info.Name, out Drive? value))
                return value;
        ...
        public Drive GetDrive(FileSystemInfo info)
        {
            string driveLetter = info.GetDriveLetter() ?? string.Empty;
            return Drives.TryGetValue(driveLetter, out Drive? value) ? value : GetDrive(new DriveInfo(driveLetter));
        }
        ...
        public Models.File[] Find(FileInfo file)
        {
            ...
            ArgumentOutOfRangeException.ThrowIfNullOrEmpty(file.DirectoryName , nameof(file.DirectoryName));
        ...
        public int Scan(IEnumerable<FileInfo> files, bool computeHash = false)
        {
            ...
            foreach (var file in files)
            {
                var found = Database.Find(file, GetDrive);
                if (!found.Any())
                {
                    var drive = GetDrive(file);
                    var item = new Models.File(file)
