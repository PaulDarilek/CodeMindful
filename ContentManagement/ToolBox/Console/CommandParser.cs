...
{
    public TextReader StdIn { get; set; } = System.Console.In;
    public TextWriter StdOut { get; set; } = System.Console.Out;
    public TextWriter StdErr { get; set; } = System.Console.Error;
    ...
    public CommandParser(IEnumerable<CommandType> validCommands)
    {
        ...
        if (!ValidCommands.ContainsKey(nameof(NoPrompt)))
        {
            var noPrompt = new CommandType(nameof(NoPrompt), (x) => NoPrompt = true) { MinParms = 0, MaxParms = 0, Description = "Stop Prompting for Command Actions" };
            ValidCommands.TryAdd(noPrompt.Name, noPrompt);
        }

        if (!ValidCommands.ContainsKey(nameof(Help)))
        {
            var noPrompt = new CommandType(nameof(NoPrompt), (x) => NoPrompt = true) { MinParms = 0, MaxParms = 0, Description = "Stop Prompting for Command Actions" };
            ValidCommands.TryAdd(noPrompt.Name, noPrompt);

            var help = new CommandType(nameof(Help), (x) => Help()) { MinParms = 0, MaxParms = 0, Description = "Display Help" };
            ValidCommands.TryAdd(help.Name, help);
        }
    }

    public CommandParser Run(string[] args, Action<Exception>? onException = null)
    {
        Parse(args);
        PromptForActions();

        foreach (var command in ExecuteAll(onException))
        {
    ...
    {
        if (NoPrompt || Actions.Any(x => x.Name == '-' + nameof(NoPrompt)))
            return;
        ...
        while (true)
        {
            ...
            if (ValidCommands.TryGetValue(text, out CommandType? command))
            {
                ...
                if(action.Name == '-' + nameof(NoPrompt))
                {
                    break;
                }
    ...
    {
        StdOut.WriteLine($"\t{action.Name} : {action.Description}");
        ...
        while (action.Parameters.Count < action.MaxParms)
        {
            ...
            if (string.IsNullOrWhiteSpace(text))
                break;
            action.Parameters.Add(text.Trim());
        }
    }
    ...
    private void LogException(Exception ex) => StdErr.WriteLine(ex.ToString());
