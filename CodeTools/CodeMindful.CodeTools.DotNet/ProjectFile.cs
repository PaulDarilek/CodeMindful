...
public class ProjectFile
{
    ...
    private void ParseFileContents()
    {
        ...
        try
        {
            ...
            if (projInfo != null)
            {
                ProjectGuid = projInfo.ProjectGuid;
                AssemblyName = projInfo.AssemblyName;
                Platform = projInfo.Platform;
                OutputType = projInfo.OutputType;
                RootNamespace = projInfo.RootNamespace;
                TargetFrameworkVersion = projInfo.TargetFrameworkVersion;
                FileAlignment =  int.Parse(projInfo.FileAlignment);
            ...
            if(refererences.Any())
            {
                Dependencies.AddRange(refererences);
            }
    ...
    public static ProjectFile ParseFromSolutionLine(string line, string rootPath)
    {
        ...
        if (line.StartsWith(projStart))
        {
            ...
            if (parts.Length >= 3)
            {
                ...
                return new ProjectFile(Path.Combine(rootPath, path)) { ProjectGuid = guid2, AssemblyName = name };
            }
        }
        return null;   
    }
